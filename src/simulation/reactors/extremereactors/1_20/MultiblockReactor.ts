...
import ReactorLogic from "./ReactorLogic";

class ModeratorNone implements IRadiationModerator {
    moderateRadiation(irradiationData: IrradiationData, radiation: RadiationPacket): void {}
}

class ModeratorAir implements IRadiationModerator {
    moderateRadiation(data: IrradiationData, packet: RadiationPacket): void {
        // applyModerator(data, packet, Moderator.AIR); // TODO: Do this
    }
}

export default class MultiblockReactor implements IReactorMachine, IReactorEnvironment {
    private static readonly REACTOR_HEAT_LOSS_CONDUCTIVITY = 0.001;

    private static readonly MODERATOR_NONE = new ModeratorNone();
    private static readonly MODERATOR_AIR = new ModeratorAir();

    private _logic: ReactorLogic;
    private _variant: IMultiblockReactorVariant;
    private _fuelHeat: Heat;
    private _reactorHeat: Heat;
    private _fuelContainer: FuelContainer;
    private _fluidContainer: FluidContainer;

    private _fuelRodsLayout: FuelRodsLayout;
    private _mode: OperationalMode;
    private _active: boolean;
    private _reactorVolume: number;
    private _fuelToReactorHeatTransferCoefficient: number;
    private _reactorToCoolantSystemHeatTransferCoefficient: number;
    private _reactorHeatLossCoefficient: number;
    private _sendUpdateFuelRodsLayout: boolean;
    private _sendUpdateFuelRodsLayoutDelayedRunnable: () => void;

    private _attachedTickables: ITickableMultiblockPart[] = [];
    private _attachedControlRods: ReactorControlRodEntity[] = [];
    private _attachedFuelRods: FuelRodsMap;

    public MultiblockReactor(variant: IMultiblockReactorVariant) {
        ...
        this._fuelToReactorHeatTransferCoefficient = 0;
        this._reactorToCoolantSystemHeatTransferCoefficient = 0;
        this._reactorHeatLossCoefficient = 0;

        this._attachedFuelRods = new FuelRodsMap();

        this._logic = new ReactorLogic(this, this.getEnergyBuffer());

        this._sendUpdateFuelRodsLayoutDelayedRunnable = () => {}; //! TODO: FIX
